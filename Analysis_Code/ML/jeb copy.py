...
def DataPipeline(filepath, ptm):
    ...
    dat_drop = drop_feat(dat_labeled, ['Eta', 'Phi', 'p_T', 'Angularity-NW', 'N-Trk', 'p_T_3', 'p_T_4', 'p_T_5', 'distmatch', 'XMatch'])
    ...
    for rad in R:
        for ptm in pThardmin:
            ...
            for feats in zip(*feat_imp_dist):
                plt.scatter(np.ones(len(feats))*i+(np.random.randn(len(feats))*0.05), feats, c=rgb_cycle, s=100, alpha=0.2)
                i+=1
            plt.xticks(np.linspace(0, len(X.columns)-1, len(X.columns)), X.columns, fontsize=30)
            plt.yticks(np.linspace(0,1,10), fontsize=30)
            plt.title("Distribution of Feature Importances in Random Forest", fontsize=40)
            plt.xlabel("Features", fontsize=35)
            plt.ylabel("Feature Importance", fontsize=35)

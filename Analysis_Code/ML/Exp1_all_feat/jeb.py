    ...
    for rad in R:
        for ptm in pThardmin:
            ...
            for feats in zip(*feat_imp_dist):
                ...
                i+=1
            plt.xticks(np.linspace(0, len(X.columns)-1, len(X.columns)), X.columns, fontsize=20)
            plt.title("Distribution of Feature Importances in Random Forest", fontsize=25)
